...
public class MemberController {
    ...
    public ResponseEntity<MemberDto> create(@RequestBody @Validated MemberForm form){// JSON 형식으로 데이터를 전송, 이를 Spring 컨트롤러에서 자바 객체로 변환
        MemberDto memberDto = MemberDto.of(form); //member 생성
        memberService.createUser(memberDto); // 여기서 중복 이메일 캐ㅍ
        return ResponseEntity.ok(memberDto); // 회원가입이 끝나면
    }
